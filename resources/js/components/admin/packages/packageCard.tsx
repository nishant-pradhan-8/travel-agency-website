...
const PackageCard: React.FC<{
    ...
    return (
        <div className="flex min-h-[250px] w-full gap-4 overflow-hidden rounded-xl border-[1px] border-gray-300 bg-white p-4">
            {/* Left Image */}
            <div className="w-1/3">
                {packageInfo.image ? (
                    <img
                        src={typeof packageInfo.image === 'string' ? `${env.APP_URL}:8000/storage/${packageInfo.image}` : ''}
            ...
            <div className="flex w-2/3 flex-col justify-between">
                ...
                <div>
                  ...
                  <button onClick={()=>onDelete(packageInfo)} ><DeleteIcon className='text-teal-800' /></button>
                  </div>
                ...
                <div className="i mt-4 flex flex-col gap-2">
                    <div>
                        <button onClick={onView} className="rounded-lg bg-teal-600 px-4 py-2 text-white transition-colors hover:bg-teal-700">
                            View Package Info
                        </button>
                    </div>
